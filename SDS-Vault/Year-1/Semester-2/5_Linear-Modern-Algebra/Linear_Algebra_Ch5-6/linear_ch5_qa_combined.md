...
**1.** What is A(V) and what is its dimension if dim V = n?
    ...
    Let V be a vector space over a field K. We will write A(V), instead of Hom(V, V), for the space of all linear mappings from V into itself. A(V) is a vector space over K, and if dim V = n, then dim A(V) = n². A(V) is an associative algebra over K with respect to composition of mappings. If dim V = n, then dim A(V) = n².
